...
public sealed class SplineComponent : Component, Component.ExecuteInEditor, Component.IHasBounds
{
	...
	[Property]
	public float SnapHeight = 10000f;

	[Property]
	public float SnapOffset = 1f;
	
	[Property, Hide]
	public Sandbox.Spline Spline = new();

	public SplineComponent()
	{
		Spline.InsertPoint( Spline.PointCount, new Sandbox.Spline.Point { Position = new Vector3( 0, 0, 0 ) } );
		Spline.InsertPoint( Spline.PointCount, new Sandbox.Spline.Point { Position = new Vector3( 100, 0, 0 ) } );
		Spline.InsertPoint( Spline.PointCount, new Sandbox.Spline.Point { Position = new Vector3( 100, 100, 0 ) } );
	}
	...
	public void SnapToGround()
	{
		...
		for ( int i = 0; i < Spline.PointCount; i++ )
		{
			...
			// Convert local spline point to world space for the trace
			var worldPos = WorldTransform.PointToWorld( point.Position );

			var start = worldPos + Vector3.Up * SnapHeight;
			var end   = worldPos + Vector3.Down * SnapHeight;
			...
			// Convert hit position back to local spline space
			point.Position = WorldTransform.PointToLocal( tr.HitPosition + Vector3.Up * SnapOffset );

			Spline.UpdatePoint( i, point );
		}
